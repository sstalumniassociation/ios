    ...
    var body: some View {
        Form {
            ...
            Section {
                Button {
                    Task.detached {
                        await userManager.beginSignUp(email: email, password: password)
                    }
                } label: {
                    Text("Sign Up")
                }
                .disabled(!(password == confirmPassword && isPasswordValid && !email.isEmpty))
            }
        }
    }
}
